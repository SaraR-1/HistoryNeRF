...
if __name__ == "__main__":
    ...
    for pose_idx in [0, 5, 10, 15, 20, 25, 30, 35, 40]:
        ...
        plt.imsave(f"./test_{pose_idx}.png", image[0])
        ...
        plt.imsave(f"./test_gt_{pose_idx}.png", image_gt)
        # Downscale the image
    ...
    print(psnr_values.mean())
    print(ssim_values.mean())
    print(lpips_values.mean())
    ...
    # cameras_val=pipeline.datamanager.eval_dataset.cameras
    # # images_val = pipeline.datamanager.eval_dataset.images
    # images_val, _ = render_trajectory(
    #     pipeline=pipeline,
    #     cameras=cameras_val,
    #     rgb_output_name = "rgb",
    #     depth_output_name = "depth",
    #     rendered_resolution_scaling_factor = 1.0,
    # )
    # images_val = torch.cat([switch_image(img_val) for img_val in images_val], dim=0)

    # breakpoint()
    
    # psnr_values = psnr(images_val, images_gt[validation_mask])
    # ssim_values = ssim(images_val, images_gt[validation_mask])
    # lpips_values = Tensor([lpips(images_val[i].unsqueeze(0).float(), images_gt[validation_mask][i].unsqueeze(0).float()).item() for i in range(images_val.shape[0])])
    # # print(psnr_values)
    # # print(ssim_values)
    # # print(lpips_values)
    ...
    # rnd_idx = torch.randperm(images_val.shape[0])
    # # images_gt[validation_mask][rnd_idx]
    # psnr_values = psnr(images_val, images_gt[validation_mask][rnd_idx])
    # ssim_values = ssim(images_val, images_gt[validation_mask][rnd_idx])
    # lpips_values = Tensor([lpips(images_val[i].unsqueeze(0).float(), images_gt[validation_mask][rnd_idx][i].unsqueeze(0).float()).item() for i in range(images_val.shape[0])])
    # # print(psnr_values)
    # # print(ssim_values)
    # # print(lpips_values)
